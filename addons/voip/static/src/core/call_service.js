...
export class CallService {
  ...
  async create(dataCall) {
    const data = this.formatPhoneNumber(dataCall);
    ...
    if (!call.partner) {
      this.orm
        .call("voip.call", "get_contact_info", [[call.id]])
        .then((partnerData) => {
          if (partnerData) {
  ...
  formatPhoneNumber(obj) {
    ...
    if (formattedObj.phone_number) {
      formattedObj.phone_number = formattedObj.phone_number
        .replace(/^\+57\s*/, "")
        .replace(/\s+/g, "");
    }
  ...
  async start(call) {
    ...
    const computeDuration = () => {
      call.timer.time = Math.floor(
        (luxon.DateTime.now() - call.startDate) / 1000
      );
    };
    computeDuration();
